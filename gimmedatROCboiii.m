...
[label_svm,score_svm] = resubPredict(mdlSVM);
...
[Xsvm,Ysvm,Tsvm,AUCsvm] = perfcurve(label_svm,score_svm,1);

%% Plot the ROC curves on the same graph.
figure
plot(Xlog,Ylog, 'r')
legend('Logistic Regression','Location','Best')
...
xlabel('False positive rate')
ylabel('True positive rate')
title('ROC Curve for Classification by Classification Trees PLUS')
hold on
